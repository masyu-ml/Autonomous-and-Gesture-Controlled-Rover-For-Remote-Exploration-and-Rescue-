...
ESP12E_SENSOR_IP   = "192.168.0.202"
SENSOR_WS_URL      = f"ws://{ESP12E_SENSOR_IP}:81/"

MODEL_PATH         = r"C:\Rover\yolov26custom.pt"
...
CLAW_ZONE         = (20, 300, 600, 240) #(20, 300, 600, 240) for VGA | (29, 310, 650, 300) for SVGA
...
# Pre-computed red HSV bounds as numpy arrays
...
class UDPCameraReceiver:
    ...
    def _recv_loop(self):
        ...
        while self.running:
            ...
            # Robust RAM protection against fragmented packets
            if ccount > 200 or t_size > 200_000 or fid <= last_complete:
                continue
...
class RoverController(QObject):
    ...
    def start(self):
        ...
        self._sensor_thread  = threading.Thread(target=self._sensor_ws_recv_loop, daemon=True, name="SensorWSRecv") # 🚨 NEW

        self._motor_thread.start()
        self._vision_thread.start()
        self._ws_recv_thread.start()
        self._sensor_thread.start() # 🚨 NEW

    def stop(self):
        if not self.running: return
        self.log_message.emit("[CTRL] Stopping...")
        self.running = False
        # 🚨 NEW: Add _sensor_thread to the thread pool shutdown
        for t in (self._motor_thread, self._vision_thread, self._ws_recv_thread, self._sensor_thread):
        ...
        with self._ws_lock:
            if self.ws:
                ...
                self.ws = None
            if self.sensor_ws: # 🚨 NEW: Close ESP-12E connection
    ...
    # 🚨 NEW: SENSOR NON-BREAKING LOOP
    def _sensor_ws_recv_loop(self):
        """ NON-BREAKING BACKGROUND THREAD FOR ULTRASONIC SENSOR """
        while self.running:
            if self.sensor_ws is None:
                try:
                    ws = websocket.WebSocket()
                    ws.connect(SENSOR_WS_URL, timeout=2) # Quick timeout so it doesn't hang
                    self.sensor_ws = ws
                    self.log_message.emit("[SENSOR] Ultrasonic ESP-12E Connected ✅")
                except:
                    time.sleep(2.0) # Fail silently and retry in 2 seconds
                    continue
            ...
            try:
                ...
                if msg:
                    ...
                    if msg_str == "WALL_ALERT":
                        ...
                        # Instantly slam the brakes if currently moving forward
                ...
                # Connection lost. Reset state safely.
                self.sensor_ws = None
                self.wall_detected = False
                time.sleep(1.0)
    ...
    def _send_motor(self, cmd: str):
        ...
        # 🚨 NEW: THE SAFETY OVERRIDE GUARD
        if getattr(self, 'wall_detected', False) and cmd == CMD_FORWARD:
            cmd = CMD_STOP # Block forward, but allow turns/reverse!
    ...
    def _vision_loop(self):
        ...
        # OPTIMIZATION: Use FP16 only if GPU is present to prevent CPU crashes/lag
        use_half_precision = (YOLO_DEVICE != "cpu")
        ...
        while self.running:
            ...
            try:
                ...
                # --- High-Speed YOLO inference (No plot overhead) ---
                results = model.predict(frame, verbose=False, imgsz=416,
                                        device=YOLO_DEVICE, conf=YOLO_CONFIDENCE, half=use_half_precision)
                annotated = frame

                r = results[0]
                if r.boxes is not None and len(r.boxes.xyxy) > 0:
                    # Manually extract and draw ONLY the highest confidence box (Saves ~15ms per frame)
                    ...
                    cv2.rectangle(annotated, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 0), 2, cv2.LINE_AA)
                    cv2.circle(annotated, obj_center, 6, (0, 255, 0), -1, cv2.LINE_AA)
                    cv2.line(annotated, CLAW_CENTER_POINT, obj_center, (0, 255, 255), 2, cv2.LINE_AA)
                    cv2.putText(annotated, f"d={dist}", (CLAW_CENTER_X + 10, CLAW_CENTER_Y + 20),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2, cv2.LINE_AA)

                # --- High-Speed Red Tape Detection ---
                ...
                if contours:
                    ...
                    if cv2.contourArea(c) > 800:
                        ...
                        if M["m00"] != 0:
                            ...
                            cv2.circle(annotated, red_center, 8, (0, 0, 255), -1, cv2.LINE_AA)
                ...
                with self.lock:
                    ...
                    s_label = self.state_label

                # --- Final HUD Overlay (Anti-Aliased) ---
                cv2.rectangle(annotated, (cz_x, cz_y), (cz_x + cz_w, cz_y + cz_h), (255, 0, 0), 2, cv2.LINE_AA)
                cv2.circle(annotated, CLAW_CENTER_POINT, 6, (0, 0, 255), -1, cv2.LINE_AA)
                grab_col = (0, 255, 0) if self._grab_state == "CONFIRMED" else (0, 165, 255)

                # Dark background for text readability
                cv2.rectangle(annotated, (5, 5), (320, 100), (0,0,0), -1)
                cv2.putText(annotated, f"STATE: {s_label}",       (12, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2, cv2.LINE_AA)
                cv2.putText(annotated, f"GRAB:  {self._grab_state}",(12, 56), cv2.FONT_HERSHEY_SIMPLEX, 0.7, grab_col,    2, cv2.LINE_AA)
                with self._tof_lock: tof_val = self._tof_mm
                if tof_val is not None:
                    cv2.putText(annotated, f"TOF:   {tof_val:.0f}mm", (12, 84), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2, cv2.LINE_AA)
    ...
    def _motor_loop(self):
        ...
        SEARCH_SEQUENCE = [
            ("RIGHT", SEARCH_TURN_DURATION), ("STOP", SEARCH_STOP_DURATION),
            ("RIGHT", SEARCH_TURN_DURATION), ("STOP", SEARCH_STOP_DURATION),
            ("RIGHT", SEARCH_TURN_DURATION), ("STOP", SEARCH_STOP_DURATION),
            ("RIGHT", SEARCH_TURN_DURATION), ("STOP", SEARCH_STOP_DURATION),
            ("RIGHT", SEARCH_FORWARD_DURATION), ("STOP", SEARCH_STOP_DURATION),
        ...
        try:
            while self.running:
                ...
                with self.lock: info = dict(self.target_info)
                now = time.time()
                ...
                # 🚨 HARDWARE OVERRIDE: ToF Laser confirmation aborts YOLO approach
                ...
                # ---- FOLLOWING ----
                    ...
                    if info["center"] is not None:
                        ...
                        with self.lock: self.target_info["last_seen"] = now
                    else:
                ...
                # ---- ANALYZING ----
                    ...
                    rx = info["red_center"]; ry = info.get("red_contour_y")
